...
const Mediadetail = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:3001/v1/leads/media/${mediaId}`)
      .then((mediaponse) => {
        const media = mediaponse.data.media;
  ...
  useEffect(() => {
    // Add a listener for the mediaize event
    const handlemediaize = () => {
      if (window.innerWidth >= 768) {
        setToggle(false); // Close the toggle when screen size is above "md"
      }
    };
    window.addEventListener("mediaize", handlemediaize);

    // Clean up the listener when the component unmounts
    return () => {
      window.removeEventListener("mediaize", handlemediaize);
    };
  }, []);
  ...
  return (
    <>
      ...
      <div className="w-full h-auto">
        <img
          src={`http://localhost:3001/${media?.cover}`} // Replace with the correct URL
          alt={media?.title}
          ...
          <div className="flex flex-col h-auto m-0">
            <div>
              ...
              {/* <div className="px-6 md:mx-10"> */}
                <div className="text-3xl font-bold lg:mt-6 mt-4 px-6 mx-6"> {media.title}</div>
